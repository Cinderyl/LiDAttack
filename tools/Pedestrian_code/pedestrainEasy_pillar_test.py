...
folder_path1 = "../input_data输入/八个顶点/Pedestrian/Easy_test"
folder_path2="../input_data输入/提取的点云/Pedestrian/Easy_test"
# 获取文件夹中所有文件的文件名列表
...
def fitness_function(pred_scores, pred_labels, points, individual):
    ...
    # 0.485-1没有识别到该物体或者识别到该物体了但是置信度小于0。3
        ...
        return (1-pred_scores)+ beta * (1 / (1 + Z)) + 1
    # 0.515-1
    # 识别成了其他物体
    else:
        return pred_scores + beta * (1 / (1 + Z)) + 3
...
def parse_config(file_number):
    ...
    parser.add_argument('--data_path', type=str, default= os.path.join("/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/data/kitti/training/velodyne_test", f"{file_number}.bin"),
                        help='specify the point cloud data file or directory')
    parser.add_argument('--per_data_path', type=str, default=os.path.join("/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/tools/final_result结果/中间bin/pointpillar/1000/20个扰动点/Pedestrian/Easy", f"{file_number}.bin"),
                        help='specify the point cloud data file or directory')
    parser.add_argument('--ckpt', type=str, default='/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/output/cfgs/kitti_models/pointpillar/default/ckpt/checkpoint_epoch_50.pth', help='specify the pretrained model')
...
def attack_GA1(points,perturbations,args,cfg,logger,first_pred_boxes,file_number):
    ...
    with open(os.path.join("/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/tools/final_result结果/中间bin/pointpillar/1000/20个扰动点/Pedestrian/Easy", f"{file_number}.bin"), 'wb') as f:
...
def main():
    ...
    for txt_file in txt_files:
        ...
        passPath = "/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/tools/final_result结果/最优扰动点/pointpillar/1000/20个扰动点/Pedestrian/Easy"
        # 获取文件夹中所有文件的文件名列表
        ...
        for j in range(20):
            ...
            # 在数组中随机选取shi行
            random_rows = np.random.choice(perturbation_4lie_zhi5.shape[0], size=20, replace=True)
        ...
        output_file=os.path.join("/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/tools/final_result结果/输出控制台结果/pointpillar/1000/20个扰动点/Pedestrian/Easy", txt_file)
        number_bin += 1
        ...
        best_individual_file=os.path.join("/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/tools/final_result结果/最优扰动点/pointpillar/1000/20个扰动点/Pedestrian/Easy", txt_file)
        np.savetxt(best_individual_file, best_individual)
        max_fitness_file=os.path.join("/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/tools/final_result结果/每一次迭代最大适应度值绘制/pointpillar/1000/20个扰动点/Pedestrian/Easy", txt_file)
        ...
        plt.xlabel('iter_num')
        plt.legend()
        plt.savefig(os.path.join("/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/tools/final_result结果/每一次迭代最大适应度值绘制/pointpillar/1000/20个扰动点/Pedestrian/Easy", f"{file_number}.jpg"))
        print(file_number)
        if(number_bin==100):
            break
