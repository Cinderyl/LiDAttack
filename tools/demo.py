...
box=np.loadtxt('边界框/446.txt')
car_points = np.loadtxt('提取.txt')
...
def parse_config():
    ...
    parser.add_argument('--data_path', type=str, default='/data0/benke/ldx/openpcdet37/OpenPCDet/data/kitti/training/velodyne/000446.bin',
                        help='specify the point cloud data file or directory')
    parser.add_argument('--per_data_path', type=str, default='/data0/benke/ldx/openpcdet37/OpenPCDet/data/KITTI/object/training/velodyne/000446.bin',
                        help='specify the point cloud data file or directory')
...
def attack_GA1(points,perturbations,args,cfg,logger,first_pred_boxes):
    ...
    with open('/data0/benke/ldx/openpcdet37/OpenPCDet/data/KITTI/object/training/velodyne/000446.bin', 'wb') as f:
    ...
    np.savetxt('per_points_xyz.txt', per_points_xyz)
    np.savetxt('per_points.txt',per_points)

    data_dict = demo_dataset.collate_batch([data_dict])
    load_data_to_gpu(data_dict)
    with torch.no_grad():
        pred_dicts, _ = model.forward(data_dict)
    pred_boxes = pred_dicts[0]['pred_boxes'].to(device)
    ...
    if (len(pred_boxes)!=0):
        ...
        for boxes in pred_boxes:
            ...
            pred_scores = pred_dicts[0]['pred_scores'][m].to(device)
            pred_labels = pred_dicts[0]['pred_labels'][m].to(device)
            m=m+1
            if dist>2:
                pred_scores = torch.tensor(9.8).to(device)
                pred_labels=torch.tensor(4).to(device)
        ...
        if(pred_scores==torch.tensor(9.8)):
            pred_scores=torch.tensor(0).to(device)
            pred_labels=torch.tensor(0).to(device)
            return pred_scores,pred_labels
            # if dist<2:
            #     print("--------------------------------------")
            #     return pred_scores,pred_labels
...
# 交叉
def uniform_crossover(population, pc,xmin,xmax,ymin,ymax,zmin,zmax):
    population_cross = []
    ...
    for i in range(0, len(population_cross), 2):
        ...
        # for j in range(0,len(population_cross[i][0]),32):
        #     a=random.random()
        #     if a < cross_prob:
        #         new_individual1 += population_cross[i][0][j:j+32]
        #         new_individual2 += population_cross[i + 1][0][j:j+32]
        #     else:
        #         new_individual1 +=population_cross[i + 1][0][j:j+32]
        #         new_individual2 +=population_cross[i][0][j:j+32]
...
# 变异
def multi_point_mutation(new_population, mutation_rate,xmin,xmax,ymin,ymax,zmin,zmax):
    ...
    for i in range(len(new_population)):
        ...
        for j in range(len(new_population[i][0])):
            ...
            else:new_individual+=new_population[i][0][j]
        T=True
        while(T):
            T=False
...
def main():
    ...
    print('置信度:', pred_scores)
    print('类别:', pred_labels)
    labels1=pred_labels
    ...
    for j in range(20):
        ...
        # for p in perturbation_4lie_zhi5:
        #     if(p[0]>xmin and p[0]<xmax and p[1]>ymin and p[1]<ymax and p[2]>zmin and p[2]<zmax):
        #         continue
        #     else:
        #         random_rows = np.random.choice(perturbation_4lie_zhi5.shape[0], size=20, replace=False)
        #         perturbation_4lie_zhi5 = perturbation_4lie_zhi5[random_rows]
        ...
        perturbation_filename = f'only_perturbation/perturbation_{j}.txt'
        np.savetxt(perturbation_filename, perturbation_4lie_zhi5 )
    ...
    def fitness_function(pred_scores,pred_labels,points,individual):
        ...
        elif(pred_labels==torch.tensor(0).to(device) or (pred_labels==torch.tensor(1).to(device) and pred_scores<=0.3)):
...
# 选择操作
    ...
    for i in range(max_iterations):
        ...
        # 选择操作
        selected_population = roulette_selection(new_population, fitness_values)

        # 交叉操作
        crossover_population = uniform_crossover(selected_population, pc,xmin,xmax,ymin,ymax,zmin,zmax)


        # 多点变异
        mutation_population=multi_point_mutation(crossover_population, mutation_rate,xmin,xmax,ymin,ymax,zmin,zmax)
        ...
        plt.pause(0.1)
        print(best_score)
        print("Iteration {}: best_score = {}".format(i, best_score))
    ...
    best_all_points= np.concatenate((points, best_individual), axis=0)
    with open('/data0/benke/ldx/openpcdet37/OpenPCDet/data/KITTIeval/object/training/velodyne/000446.bin', 'wb') as f:
        # 依次将每个点的四个值转换为二进制，并写入文件中
        for point in best_all_points:
            x, y, z, intensity = point
            f.write(struct.pack('ffff', x, y, z, intensity))
    ...
    print("time_sum", time_sum)
        # if best_score<fitness_threshold or pred_labels!=labels1:
        #     break
    # best_individual = population[np.argmax(fitness_values)]
    # best_score = 1.0 / max(fitness_values)
    # print("Best score = {}".format(best_score))
    # print("label={}".format(pred_labels))
