...
folder_path1 = "/home/Newdisk/liaodanxin/openpcdet37/OpenPCDet/tools/input_data输入/八个顶点/Car/easy"
folder_path2="/home/Newdisk/liaodanxin/openpcdet37/OpenPCDet/tools/input_data输入/提取的点云/Car/Easy"
# 获取文件夹中所有文件的文件名列表
...
def calculate_farthest_distance(points):
    """
    计算给定点集合中的最远距离。

    参数：
    points: 包含点坐标的列表，每个点坐标是一个元组 (x, y, z)。
...
def fitness_function(pred_scores, pred_labels, points, individual):
    ...
    distances = calculate_farthest_distance(individual)
    beta = 0.8
    alpha=1
...
def parse_config(file_number):
    ...
    parser.add_argument('--data_path', type=str, default= os.path.join("/home/Newdisk/liaodanxin/openpcdet37/OpenPCDet/data/kitti/training/velodyne", f"{file_number}.bin"),
                        help='specify the point cloud data file or directory')
    parser.add_argument('--per_data_path', type=str, default=os.path.join("/home/Newdisk/liaodanxin/openpcdet37/OpenPCDet/tools/final_result结果/中间bin/1000/10个扰动点/Car/Easy", f"{file_number}.bin"),
                        help='specify the point cloud data file or directory')
    parser.add_argument('--ckpt', type=str, default='/home/Newdisk/liaodanxin/openpcdet37/OpenPCDet/output/cfgs/kitti_models/pointrcnn/default/ckpt/checkpoint_epoch_79.pth', help='specify the pretrained model')
    parser.add_argument('--ext', type=str, default='.bin', help='specify the extension of your point cloud data file')
    args = parser.parse_args()
    cfg_from_yaml_file(args.cfg_file, cfg)
    return args, cfg
# 我要实现的功能：输入扰动点之后，强度我都设置为5，扰动点加上原始点，就得到新的点，转为.bin文件，输入到模型里面得到置信度和预测类别
def attack_GA1(points,perturbations,args,cfg,logger,first_pred_boxes,file_number):
    new_points = np.concatenate((points, perturbations), axis=0)
    with open(os.path.join("/home/Newdisk/liaodanxin/openpcdet37/OpenPCDet/tools/final_result结果/中间bin/1000/10个扰动点/Car/Easy", f"{file_number}.bin"), 'wb') as f:
...
def one_point_crossover(population, pc,xmin,xmax,ymin,ymax,zmin,zmax):
    ...
    for i in range(0, len(population_cross), 2):
        new_individual1 = ''
        new_individual2 = ''
        crossover_point1 = random.randint(1, 31)  # 随机选择交叉点
        crossover_point2 = random.randint(32, len(population_cross[i][0]) - 1)  # 随机选择交叉点
        for k in range(32):
            if k < crossover_point1:
                new_individual1 += population_cross[i][0][k]
                new_individual2 += population_cross[i + 1][0][k]
            else:
                new_individual1 += population_cross[i + 1][0][k]
                new_individual2 += population_cross[i][0][k]
        for j in range(32,len(population_cross[i][0])):
        ...
        while(T):
            ...
            for k in range(0, len(new_individual1), 96):  # 81120的长度96乘以845
                ...
                if (x1 < xmax and x1 > xmin and y1 < ymax and y1 > ymin and z1 < zmax and z1 > zmin and x2 < xmax and x2 > xmin and y2 < ymax and y2 > ymin and z2 < zmax and z2 > zmin):
                    ...
                    new_individual2 = ''
                    crossover_point1 = random.randint(1, 31)  # 随机选择交叉点
                    crossover_point2 = random.randint(33, len(population_cross[i][0]) - 1)  # 随机选择交叉点
                    for k1 in range(32):
                        if k1 < crossover_point1:
                            new_individual1 += population_cross[i][0][k1]
                            new_individual2 += population_cross[i + 1][0][k1]
                        else:
                            new_individual1 += population_cross[i + 1][0][k1]
                            new_individual2 += population_cross[i][0][k1]
                    for j in range(32, len(population_cross[i][0])):
                        if j < crossover_point2:
                            new_individual1 += population_cross[i][0][j]
                            new_individual2 += population_cross[i + 1][0][j]
    ...
    for i in range(len(new_population)):
        ...
        for k1 in range(32):
            ...
            if c < non_uniform_rate:
                if new_population[i][0][k1] == '1':
                    new_individual += '0'
                else:
                    new_individual += '1'
            else:
                new_individual += new_population[i][0][k1]
        for j in range(32,len(new_population[i][0])):
            c = random.random()
            if c < non_uniform_rate:
...
def main():
    ...
    for txt_file in txt_files:
        ...
        passPath = "/home/Newdisk/liaodanxin/openpcdet37/OpenPCDet/tools/final_result结果/最优扰动点/1000/10个扰动点/Car/Easy"
        # 获取文件夹中所有文件的文件名列表
        ...
        perturbation_4lie = []
        num_samples = 20
        num_perturbed_points = 10
        for j in range(num_samples):
            # 生成随机扰动
            # perturbation = np.random.normal(0, 0.01, size=(1, 3))  # 使用标准正态分布生成扰动
            outer_range_x = (xmin, xmax)
            random_x = np.random.uniform(outer_range_x[0], outer_range_x[1])
            # 构造扰动后的点云，使其 x 轴坐标相同
            perturbed_points = np.copy(car_points3)
            perturbed_points[:, 0] =random_x  # 在所有点的 x 轴坐标上加上相同的扰动值

            # 选择 num_perturbed_points 个扰动点的索引
            selected_indices = np.random.choice(perturbed_points.shape[0], size=num_perturbed_points, replace=True)

            # 取出这些扰动点
            selected_perturbed_points = perturbed_points[selected_indices]

            # 增加多一列
            # 新的一列，值为 intensity1
            new_column = np.full((selected_perturbed_points.shape[0], 1), intensity1)

            # 将新列和原始数组拼接起来
            perturbation_4lie_zhi5 = np.concatenate((selected_perturbed_points, new_column), axis=1)
        ...
        for individual in new_population:
            pred_scores, pred_labels = attack_GA1(points, individual, args, cfg, logger, first_pred_boxes, file_number)
            f = fitness_function(pred_scores, pred_labels, car_points, individual)
            f = f.cpu().numpy()
            fitness.append(f)
        ...
        output_file=os.path.join("/home/Newdisk/liaodanxin/openpcdet37/OpenPCDet/tools/final_result结果/输出控制台结果/1000/10个扰动点/Car/Easy", txt_file)
        number_bin += 1
        ...
        best_individual_file=os.path.join("/home/Newdisk/liaodanxin/openpcdet37/OpenPCDet/tools/final_result结果/最优扰动点/1000/10个扰动点/Car/Easy", txt_file)
        np.savetxt(best_individual_file, best_individual)
        max_fitness_file=os.path.join("/home/Newdisk/liaodanxin/openpcdet37/OpenPCDet/tools/final_result结果/每一次迭代最大适应度值绘制/1000/10个扰动点/Car/Easy", txt_file)
        ...
        plt.xlabel('iter_num')
        plt.legend()
        plt.savefig(os.path.join("/home/Newdisk/liaodanxin/openpcdet37/OpenPCDet/tools/final_result结果/每一次迭代最大适应度值绘制/1000/10个扰动点/Car/Easy", f"{file_number}.jpg"))
        print(file_number)
        if(number_bin==3):
            break
