...
folder_path1 = "../input_data1/八个顶点/Car/easy"
folder_path2="../input_data1/提取的点云/Car/Easy"
# 获取文件夹中所有文件的文件名列表
...
def plane_distance_constraint(points):
    """
    计算点集合到垂直于 x 轴的平面的距离约束。
    ...
    for x, y, z in points:
        # 计算点到垂直 x 轴的平面的距离（即 x 轴坐标）
        distance_to_plane = abs(x)
        constraint_sum += distance_to_plane
...
def fitness_function(pred_scores, pred_labels, points, individual):
    ...
    plane_distance=plane_distance_constraint(individual)
    beta = 0.4
    alpha=1
    ...
    # 0.485-1没有识别到该物体或者识别到该物体了但是置信度小于0。3
        ...
        return (1-pred_scores)+ beta * (1 / (1 + distances)) +alpha*(1/(1+plane_distance))+1
    # 0.515-1
    # 识别成了其他物体
    else:
        return pred_scores + beta * (1 / (1 + distances)) + alpha*(1/(1+plane_distance))+4
...
def parse_config(file_number):
    ...
    parser.add_argument('--data_path', type=str, default= os.path.join("/data0/benke/ldx/openpcdet37/OpenPCDet/data/kitti/training/velodyne", f"{file_number}.bin"),
                        help='specify the point cloud data file or directory')
    parser.add_argument('--per_data_path', type=str, default=os.path.join("/data0/benke/ldx/openpcdet37/OpenPCDet/tools/final_result结果1/中间bin/500/10个扰动点/Car/Easy", f"{file_number}.bin"),
                        help='specify the point cloud data file or directory')
    parser.add_argument('--ckpt', type=str, default='/data0/benke/ldx/openpcdet37/OpenPCDet/output/cfgs/kitti_models/pointrcnn/default/ckpt/pointrcnn_7870.pth', help='specify the pretrained model')
...
def attack_GA1(points,perturbations,args,cfg,logger,first_pred_boxes,file_number):
    ...
    with open(os.path.join("/data0/benke/ldx/openpcdet37/OpenPCDet/tools/final_result结果1/中间bin/500/10个扰动点/Car/Easy", f"{file_number}.bin"), 'wb') as f:
...
def x_crossover(population, pc,xmin,xmax,ymin,ymax,zmin,zmax):
    population_cross = []
    for individual in population:
        individual_3=individual[:,:3]
        binary=''
...
def main():
    ...
    for txt_file in txt_files:
        ...
        passPath = "/data0/benke/ldx/openpcdet37/OpenPCDet/tools/final_result结果1/最优扰动点/500/10个扰动点/Car/Easy"
        # 获取文件夹中所有文件的文件名列表
        ...
        for j in range(20):
            # 生成随机扰动
            perturbation = np.random.normal(0, 0.01, size=car_points3.shape)  # 使用标准正态分布生成与点云数据形状相同的随机扰动
            perturbed_points = car_points3 + perturbation
        ...
        # for _ in range(20):
        #     # 随机选择一个 x 坐标
        #     random_x = random.uniform(min(x for x, _, _ in car_points3), max(x for x, _, _ in car_points3))
        #     perturbed_points = []
        #     for _ in range(10):
        #         y = random.uniform(min(y for _, y, _ in car_points3), max(y for _, y, _ in car_points3))
        #         z = random.uniform(min(z for _, _, z in car_points3), max(z for _, _, z in car_points3))
        #         random_point = (random_x, y, z)
        #         perturbed_points.append(random_point)
        #
        #     # 增加多一列
        #     # 新的一列，值为5
        #     new_column = np.full((perturbed_points.shape[0], 1), intensity1)
        ...
        #     perturbation_4lie.append(perturbation_4lie_zhi5)
        #     new_population = perturbation_4lie
        #     generated_point_clouds.append(random_points)
        ...
        output_file=os.path.join("/data0/benke/ldx/openpcdet37/OpenPCDet/tools/final_result结果1/输出控制台结果/500/10个扰动点/Car/Easy", txt_file)
        number_bin += 1
        ...
        best_individual_file=os.path.join("/data0/benke/ldx/openpcdet37/OpenPCDet/tools/final_result结果1/最优扰动点/500/10个扰动点/Car/Easy", txt_file)
        np.savetxt(best_individual_file, best_individual)
        max_fitness_file=os.path.join("/data0/benke/ldx/openpcdet37/OpenPCDet/tools/final_result结果1/每一次迭代最大适应度值loss绘制/500/10个扰动点/Car/Easy", txt_file)
        ...
        plt.xlabel('iter_num')
        plt.legend()
        plt.savefig(os.path.join("/data0/benke/ldx/openpcdet37/OpenPCDet/tools/final_result结果1/每一次迭代最大适应度值loss绘制/500/10个扰动点/Car/Easy", f"{file_number}.jpg"))
        print(file_number)
        if(number_bin==3):
            break
