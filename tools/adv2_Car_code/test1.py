...
# 假设我们的状态是对抗样本的误差，范围是[0, 1]，我们可以将其分为100个bin。
# 行动是选择一个对抗样本。假设我们在每次迭代中都能生成10个对抗样本。
# 所以，Q-table的大小为100x10。
state_size = 100
action_size = 20
...
def parse_config(file_number):
    ...
    parser.add_argument('--per_data_path', type=str, default=os.path.join("/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/tools/final_result结果/中间bin/pointpillar/1000/10个扰动点/adv2_Car/Easy", f"{file_number}.bin"),
                        help='specify the point cloud data file or directory')
...
def roulette_selection(population, fitness_values):
    ...
    new_population = []
    # 获取当前状态
    current_state_index = get_state_index(fitness_values)
    # 使用Q-learning选择一个行动
    action = choose_action(current_state_index)
    new_population.append(population[action])
    # 计算适应度总和
    ...
    for txt_file in txt_files:
        ...
        for i in range(max_iterations):
            ...
            for i in range(len(mutation_population )):
                temperature = initial_temperature * (cooling_rate ** generations)
                mutation_population[i] = simulated_annealing(mutation_population[i], temperature, cooling_rate)
        ...
        output_file=os.path.join("/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/tools/final_result结果/输出控制台结果/pointpillar/1000/10个扰动点/adv2_Car/Easy", txt_file)
        number_bin += 1
        ...
        best_individual_file=os.path.join("/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/tools/final_result结果/最优扰动点/pointpillar/1000/10个扰动点/adv2_Car/Easy", txt_file)
        np.savetxt(best_individual_file, best_individual)
        max_fitness_file=os.path.join("/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/tools/final_result结果/每一次迭代最大适应度值绘制/pointpillar/1000/10个扰动点/adv2_Car/Easy", txt_file)
        ...
        plt.xlabel('iter_num')
        plt.legend()
        plt.savefig(os.path.join("/home/Newdisk/liaodanxin/pcdet111/OpenPCDet/tools/final_result结果/每一次迭代最大适应度值绘制/pointpillar/1000/10个扰动点/adv2_Car/Easy", f"{file_number}.jpg"))
        print(file_number)
        if(number_bin==50):
            break
