...
folder_path1 = "../边界框/Car/easy"
folder_path2="../提取的点/Car/easy"
# 获取文件夹中所有文件的文件名列表
...
def fitness_function(pred_scores, pred_labels, points, individual):
    ...
    if (pred_labels == torch.tensor(1).to(device) and pred_scores > 0.5):
    ...
    # 0.485-1没有识别到该物体或者识别到该物体了但是置信度小于0。3
            ...
            pred_labels == torch.tensor(1).to(device) and pred_scores <= 0.5)):
...
def parse_config(file_number):
    ...
    parser.add_argument('--per_data_path', type=str, default=os.path.join("/data0/benke/ldx/openpcdet37/OpenPCDet/tools/空bin/Car/easy", f"{file_number}.bin"),
                        help='specify the point cloud data file or directory')
...
def attack_GA1(points,perturbations,args,cfg,logger,first_pred_boxes,file_number):
    ...
    with open(os.path.join("/data0/benke/ldx/openpcdet37/OpenPCDet/tools/空bin/Car/easy", f"{file_number}.bin"), 'wb') as f:
...
def main():
    ...
    for txt_file in txt_files:
        ...
        for i in range(max_iterations):
            ...
            # 精英加锦标赛选择操作
            selected_population = elitism_tournament_selection(new_population, fitness_values, num_elites,
                                                               tournament_size)
            ...
            score2.append(score)
            a=np.concatenate((new_population, new_population1), axis=0)

            # new_population= roulette_selection(np.concatenate((new_population, new_population1), axis=0), fitness_values)
            new_population = elitism_tournament_selection(np.concatenate((new_population, new_population1), axis=0),
                                                          fitness_values, num_elites, tournament_size)
        ...
        output_file=os.path.join("/data0/benke/ldx/openpcdet37/OpenPCDet/tools/输出控制台结果/500/扰动点10/Car/easy", txt_file)
        ...
        best_individual_file=os.path.join("../最优扰动点/500/扰动点10/Car/easy", txt_file)
        np.savetxt(best_individual_file, best_individual)
        max_fitness_file=os.path.join("../每一次迭代最大适应度值loss绘制/500/扰动点10/Car/easy", txt_file)
        ...
        plt.xlabel('iter_num')
        plt.legend()
        plt.savefig(os.path.join("../每一次迭代最大适应度值loss绘制/500/扰动点10/Car/easy", f"{file_number}.jpg"))
