...
def attack_GA1(points,perturbations,args,cfg,logger,first_pred_boxes):
    ...
    with open('/data0/benke/ldx/openpcdet37/OpenPCDet/tools/中间bin文件/000002.bin', 'wb') as f:
...
def main():
    ...
    def fitness_function(pred_scores,pred_labels,points,individual):
        ...
        # 0.485-1没有识别到该物体或者识别到该物体了但是置信度小于0。3
            ...
            return (1 - (pred_scores + 10) / 20) + beta * (1/(1+Z))+1
        # 0.515-1
        # 识别成了其他物体
        else:
            return (pred_scores + 10) / 20 + beta * (1/(1+Z))+3
...
#每一次的最大适应度值
    ...
    plt.xlabel('iter_num')
    plt.legend()
    plt.savefig(os.path.join("/data0/benke/ldx/openpcdet37/OpenPCDet/tools/每一次迭代最大适应度值loss绘制/1000/扰动点10/Car/easy", "000002.jpg"))
    ...
    output_file = "/data0/benke/ldx/openpcdet37/OpenPCDet/tools/输出控制台结果/1000/扰动点10/Car/easy/000002.txt"
    ...
    np.savetxt('../每一次迭代最大适应度值loss绘制/1000/扰动点10//Car/easy/000002.txt', max_fitness)
